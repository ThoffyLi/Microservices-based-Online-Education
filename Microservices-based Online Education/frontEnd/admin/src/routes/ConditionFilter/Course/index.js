...
class CourseConditionFilter extends Component {
  // To generate mock Form.Item
  getFields() {
    const { getFieldDecorator } = this.props.form;
    const children = [];
    //TODO: select by teacher
    // children.push(
    //   <Col span={5} key={1733}>
    ...
    children.push(
      <Col span={5} key={i++ + 1} style="block">
        <Form.Item label={`Search By Course Name`}>
  ...
  getCourseName = () => {
    const name = this.props.form.getFieldsValue().courseName;
    return name;
  };
...
export default CourseConditionFilter;
